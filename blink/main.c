...
void i2cWriteWord(uint8_t adr, uint8_t val1, uint8_t val2) {
	uint8_t v[]={ val1, val2 };
...
int main(void) {
	...
	i2cWriteWord(0x20,0x00,0x00);
	while(1) {
		for(uint8_t i=0; i<szSweep; i++) {
			i2cWriteWord(0x20,0x14,sweep[i]);
			sleep_ms(40);
		}
		for(uint8_t i=0; i<3; i++) {
			i2cWriteWord(0x20,0x14,0xFF); sleep_ms(50);
			i2cWriteWord(0x20,0x14,0x00); sleep_ms(50);
		}
